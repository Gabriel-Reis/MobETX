...
// PENALIDADE POR ALTA PORCETAGEM DE LINKS QUEBRADOS
...
public class Statistics extends App {
	...
	protected static double res_help[][] = null;
...
/*HERE*/double Local_D_spatial = 0.0;
/*HERE*/int Local_D_spatial_count = 0;
		...
		int done = 0;
		int progress = -1;
		double result[][] = new double[6][node.length];
/*HERE*/double degreeByNode[] = new double[node.length];

		for (int i = 0; i < node.length; i++) {
/*HERE*/	Local_D_spatial = 0.0;
/*HERE*/	Local_D_spatial_count = 0;
			...
			for (int j = i + 1; j < node.length; j++) {
				...
				Local_D_spatial += linkStatusChanges[2];
				Local_D_spatial_count += linkStatusChanges[3];
...
/*HERE*/	degreeByNode[i]=Local_D_spatial_count/Local_D_spatial;
		}
		...
		result[0][0] = mobility / on_time;
		result[1][0] = on_time_node;
		result[2][0] = D_spatial / D_spatial_count;
		result[3][0] = Relative_speed / Relative_speed_count;
		result[4][0] = connectedPairs;
/*HERE*/for (int i = 0; i < node.length; i++) {
			result[5][i] = degreeByNode[i];
		}
...
/*HERE*/int linkPairbreaks[][] = new int[node.length][node.length];
/*HERE*/double timelinked[][] = new double[node.length][node.length];
/*HERE*/double timelinkedVet[] = new double[node.length];
/*HERE*/double timelinkbreaks[][] = new double[node.length][node.length];
/*HERE*/double timelinkbreaksVet[] = new double[node.length];
...
/*HERE*/for (int a = 0;a<node.length;a++) {
		...
		for (int k = 0; k < radius.length; k++) {
			...
			mobility = res_help[0][0];
			on_time = res_help[1][0];
			...
			while (heap.size() > 0) {
				...
				if (ls[idx.i][idx.j - idx.i - 1] < 0.0) { // connect
					...
					linkDuration += tConn;
/*HERE*/			timelinked[idx.i][idx.j]+= tConn;
/*HERE*/			timelinked[idx.j][idx.i]+= tConn;
/*HERE*/			timelinkedVet[idx.i]+= tConn;
/*HERE*/			timelinkedVet[idx.j]+= tConn;
					links++;
/*HERE*/			linksDone++;
/*HERE*/			numlinked[idx.i][idx.j]+=1;
/*HERE*/			numlinked[idx.j][idx.i]+=1;
/*HERE*/			numlinkedVet[idx.i]+=1;
/*HERE*/			numlinkedVet[idx.j]+=1;
					if ((tNew < duration) && (tUp > 0.0)) {
						linkDurations.addElement(new Double(tConn));
						timeToLinkBreak += tConn;
/*HERE*/				timelinkbreaks[idx.i][idx.j]+= tConn;
/*HERE*/				timelinkbreaks[idx.j][idx.i]+= tConn;
/*HERE*/				timelinkbreaksVet[idx.i]+= tConn;
/*HERE*/				timelinkbreaksVet[idx.j]+= tConn;
						linkbreaks++;
/*HERE*/				linkPairbreaks[idx.i][idx.j]++;
/*HERE*/				linkPairbreaks[idx.j][idx.i]++;
/*HERE*/				linkbreaksVet[idx.i]++;
/*HERE*/				linkbreaksVet[idx.j]++;
						// rebuild pIdx
						if (partitions == 1) {
			...
			double D_spatial = res_help[2][0];
			// get relative speed
			double R_speed = res_help[3][0];
			// get the number of node pairs that were ever linked
			double connectedPairs = res_help[4][0];
			...
			// 0  - tx range 
			// 1  - avg. degree of spatial dependence
			// 2  - avg. degree
			// 3  - avg. num. of partitions
			// 4  - partitioning degree 
			// 5  - avg. time to link break
			// 6  - std. deviation of time to link break
			// 7  - link breaks
			// 8  - avg. link duration
			// 9  - total links
			// 10 - avg. relative speed
			// 11 - avg. path availability
			// 12 - avg. number of link changes
///*HERE*/	degree=res_help[5]; //Recupera grau dos nós
		}
		...
		// stats.println("No - Velocidade - DegreeOfTemporalDependence - breaks - degree");
		// for (int i = 0; i < node.length; i++) {
		// 	stats.print(i +" / ");
		// 	stats.print(averageSpeed(i, s) +" / ");
		// 	stats.print(getAverageDegreeOfTemporalDependence(node, s) +" / ");
		// 	stats.print(breaks[i] +" / ");
		// 	stats.print(res_help[5][i]);
		// 	AvgDegree+=res_help[5][i];
		// 	stats.println("\n");
		// }
